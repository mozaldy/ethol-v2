...
$sql = "SELECT * FROM materi WHERE is_tugas='1' ORDER BY uploaded_at DESC LIMIT 5";
$stmt = $conn->prepare($sql);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows > 0) {
    $materiData = $result->fetch_all(MYSQLI_ASSOC);
} else {
    $materiData = [];
}
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-start">
        ...
        <a href="{{ "/" | relURL }}" class="flex ml-2 md:mr-24">
          <img src="/kuliah/uas/assets/ethol-logo.png" class="h-14 mr-3" alt="Logo Ethol" />
        </a>
      </div>
...
<?php
// Assuming $materiData contains the data of materi uploaded by the current logged-in user
foreach ($materiData as $materi) {
    // Extract relevant materi details
    $namaMateri = htmlspecialchars($materi['nama_materi']);
    $uploadedAt = htmlspecialchars($materi['uploaded_at']);
    ?>
<div>
    <a href="#" class="flex px-4 py-3 border-b hover:bg-gray-100 dark:hover:bg-gray-600 dark:border-gray-600">
        <div class="w-full pl-3">
            <div class="text-gray-500 font-normal text-sm mb-1.5 dark:text-gray-400">
                Tugas baru: <span class="font-semibold text-gray-900 dark:text-white"><?php echo $namaMateri; ?></span>
            </div>
            <div class="text-xs font-medium text-primary-700 dark:text-primary-400"><?php echo $uploadedAt; ?></div>
        </div>
    </a>
</div>
<?php } ?>
...
?>
              ...
              <ul class="py-1" role="none">
                ...
                <li>
                  <a href="/kuliah/uas/src/dashboard/logout.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">Sign out</a>
                </li>
              </ul>
            </div>
          </div>
